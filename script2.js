document.addEventListener('DOMContentLoaded', () => {
	...
	const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbxbwJ36iAWUVXRJO06RbK8mY0BNilyCZDh4jyPYrGmgmWRNgbkz8VCFgUBrJpiDWxIc/exec";
	...
	if (campusMap) {
	    campusMap.onclick = function(e) {
	        ...
	        // Calculate percentage coordinated for responsiveness
	...
	if (feedbackForm) {
	    feedbackForm.onsubmit = function(e) {
	        ...
	        const data = {
	            ...
	            year: document.getElementById('yr-section').value,
	            room: document.getElementById('roomnum').value,
	            rating: document.querySelector('input[name="rating"]:checked')?.value,
	            suggestion: document.getElementById('feedback-area').value,
	            anonymous: document.getElementById('anon-check').checked
	        };
	        ...
	        .then(() => {
            notify(`Feedback for ${data.room} submitted successfully!`, "success");
	            addRequestToHistory("Classroom Feedback", data.room);
	            this.reset();
	            showSection('#query');
	        })
	        .catch(() => {
	            notify("Failed to submit feedback.", "danger");
	        });
	    };
	}
	...
	if (readinessForm) {
	    readinessForm.onsubmit = function(e) {
	        ...
	        const needs = [...document.querySelectorAll('input[name="necessity"]:checked')]
	            .map(el => el.value);
	        ...
	        const data = {
	            ...
	            evacuation: document.querySelector('input[name="evacpath"]:checked')?.value,
	            needs: needs.join(", "),
	        ...
	        .then(() => {
	            notify("Readiness submitted successfully!", "success");
	            addRequestToHistory("Readiness Survey", "Student Profile");
	            this.reset();
	            showSection('#dashboard');
	        })
	        .catch(() => {
	            notify("Failed to submit readiness.", "danger");
	        });
	    };
	}
